...
from httpio.request  import HTTPRequest, load_request
...
class HTTPServer:
    ...
    # ── transport hooks (plaintext defaults; SecureHTTPServer overrides these) ──
    def _handshake(self, sock: socket.socket):
        return None

    def _recv(self, sock: socket.socket, ctx):
        return self._recv_full(sock)

    def _send(self, sock: socket.socket, data: bytes, ctx):
        sock.send(data)                               # plaintext: raw bytes
    ...
    def handle(self, client: socket.socket | None = None, close: bool = True):
        ...
        try:
            ...
            warnings.warn(f"connection setup failed: {e}")
        ...
        try:
            ...
            self._send(client, HTTPResponse(status=400, message="Bad_Request").dump(), ctx)
        ...
        self._send(client, response.dump(), ctx)
