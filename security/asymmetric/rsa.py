...
class RSA:
    ...
    def receive(self, payload: bytes, sign_data: bytes = None, verify: bool = True) -> bytes:
        ...
        if verify and sign_data is not None:
            ...
            sig_payload = obj["sig"].encode()

            if not verify_bytes(sign_data, sig_payload, self.peer_public_key):
...
# ----------------------------
def test():
    print("=== RSA keys making ===")
    primes = distant_random_primes(256)
    private_key, public_key = rsa_keys(primes)

    data = secrets.randbelow(public_key.n)
    encrypted = encrypt(data, public_key)
    decrypted = decrypt(encrypted, private_key)
    print("Int OK?", decrypted == data)

    msg = b"Hello RSA bytes API!"
    enc_payload = encrypt_bytes(msg, public_key)
    dec_bytes = decrypt_bytes(enc_payload, private_key)
    print("Bytes OK?", dec_bytes == msg)

    sig_payload = sign_bytes(msg, private_key)
    print("Verify OK?", verify_bytes(msg, sig_payload, public_key))

    dumped = dump_public_key(public_key)
    loaded = load_public_key(dumped)
    print("Dump/load OK?", loaded == public_key)

def demo_communication():
    priv_a, pub_a = rsa_keys(distant_random_primes(256))
    priv_b, pub_b = rsa_keys(distant_random_primes(256))

    assi = RSA(my_public_key=pub_a, my_private_key=priv_a)
    shahaf = RSA(my_public_key=pub_b, my_private_key=priv_b)

    shahaf.load_peer_public_key(assi.dump_my_public_key())
    assi.load_peer_public_key(shahaf.dump_my_public_key())

    msg = b"Secret message!"
    payload = assi.send(msg, sign_data=msg)
    received = shahaf.receive(payload, sign_data=msg, verify=True)

    print("Comm OK?", msg == received)
